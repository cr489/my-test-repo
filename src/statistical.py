...
class stat:
    ...
    def eclud(self):
        ...
        file1 = np.zeros(
            (self.file.shape[1], len(self.file[1]))
        )  # Reading it numpy array
        for i in range(self.file.shape[1]):
            file1[i] = self.file[i].values

        file2 = np.zeros((len(file1[:-2]) // 2, len(self.file[1])))
        for j in range(len(self.file[1])):
            file2[0, j] = np.linalg.norm(file1[2:4, j])
            file2[1, j] = np.linalg.norm(file1[4:6, j])
            file2[2, j] = np.linalg.norm(file1[6:8, j])
        return file1, file2
